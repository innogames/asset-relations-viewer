...
- Require Unity 2021.1 as min Unity version
    - This due to the use of Unitys ListPool class which got added in this version

**4.3.0**

- Use NextVisible() instead of Next() to avoid recursive SerializedProperties
    - In Unity 6000.3.x uxml assets have a back reference to the parent property leading to
      children.parent.children.parent. recursion
...
- Change the cache update logic from "each cache updates for all assets" to "each asset updates every cache" which
  improves the async mode to be as fast as non async.
...
- Fix possible ```cs ArgumentException: An item with the same key has already been added``` which happened if a deleted
  file was referenced as a dependency
...
- Removed IsExisting info from nodes since caches now need to delete nodes if they are saved but not existing anymore on
  update from their node list
...
- Fixed possible StackOverflowException with very huge dependency trees and ShowAdditionalInformation option being
  enabled
...
- Fixed issue that total amount of supported assets was limited to max size of short but is now limitted to max size of
  int.
...
- Fixed bug where optimization to save calculations in the reflection stack (which can find generic dependencies) could
...
- Fixed bug where the same nodes dependencies would be added multiple times when using the AssetToFile dependency
  resolver
...
- Fixed several bugs regarding detection of if an asset is packed into the app or not and added cache to speed up the
  detection
...
- Fixed issue with dependency resolvers getting updated several times when a dependency resolver gets
  activated/deactivated in the AssetRelationsViewer
...
- ObjectSerializedDependencyResolver and AssetToFileDependencyResolver are now the default activated dependency
  resolvers
...
- Removed comparison against dependencies returned from the AssetDatabase.GetDependencies() function in the
